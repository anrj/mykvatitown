...
Lane follow + stop-sign detection. Deploy on the leader Duckiebot.

Launched by `python launch.py --run --bot <leader-hostname> --task project_leader`
"""
...
generate_raw_frames = make_frame_generator(
    lambda: camera, lambda f: f or np.zeros((480, 640, 3), dtype=np.uint8), quality=70, rgb=True)
 ...
 *{{margin:0;padding:0;box-sizing:border-box}}
 ...
 .header h1{{font-size:18px;font-weight:600}} .header .sub{{color:var(--muted);font-size:13px}}
...
function sliderChanged(k){{
  ...
  if(inp)inp.value=v; if(dv)dv.textContent=v.toFixed(2);
